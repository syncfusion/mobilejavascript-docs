...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/mobilejavascript-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/mobilejavascript-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore') {
                ...
                if (fs.existsSync('../mobilejavascript-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/mobilejavascript-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../mobilejavascript-docs/` + changedFileName, `../../../gitlabRepo/mobilejavascript-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/mobilejavascript-docs/')) {
                            shelljs.cp('-rf', `../mobilejavascript-docs/` + changedFileName, `../../../gitlabRepo/mobilejavascript-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/mobilejavascript-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/mobilejavascript-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/mobilejavascript-docs`);
